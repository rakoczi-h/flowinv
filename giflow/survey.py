...
class GravitySurvey():
    ...
    def plot_contours(self, filename='survey.png', include_noise=False, axis_limits=None):
        ...
        levels_2 = np.linspace(self.gravity.min(), self.gravity.max()+gravity_range/20, 5)
        cmap = 'plasma'
        ...
        ax.tricontour(self.survey_coordinates[:,0], self.survey_coordinates[:,1], plot_data, levels=levels_2, colors='k', linewidths=0.2)
        ...
        cbar.set_label(label=r'$\Delta$g [$\mu$Gal]', fontsize=16)
        plt.savefig(filename)
        plt.close()
