...
# -------------------- Test data  ------------------
with open(os.path.join(flow.data_location, "testset_to_present_v2_0.pkl"), 'rb') as file:
#with open("/data/wiay/2263373r/giflow/box/qinetiq_dummy_set.pkl", 'rb') as file:
    dt_test = pkl.load(file)

testsize = 4 # THIS needs to be edited to give the overall desired data set size
test_data, test_conditional = read_files(data_location=data_location, filenames=['testset_to_present_v2_0.pkl'], datasize=testsize, survey_coordinates_to_include=survey_coordinates_to_include, model_info_to_include=model_info_to_include, mix_survey_order=mix_survey_order)
