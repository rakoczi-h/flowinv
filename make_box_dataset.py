...
#n = int(sys.argv[1])
save_loc = '/scratch/balta0/2263373r/giflow/4_paper/'
...
# FRAMEWORKS
survey_framework = {'noise_scale' : 0.0288, 'survey_shape' : [8,8], 'ranges': [[-0.5,0.5],[-0.5, 0.5],[0]], 'noise_on_location_scale' : 0.025}
# variable noise scale: ['Uniform', 0.01, 0.25], single noise scale: 0.125
# noise on location 0.025
model_framework = {'type': 'voxelised', 'density': -2670.0, 'noise_scale': 0.0, 'grid_shape': [10,10,10], 'ranges': [[-0.75,0.75],[-0.75,0.75],[-0.75,0.0]]}
# noise scale: 500.0
...
#file_name = os.path.join(save_loc, f"trainset_2.pkl")
...
#dt_train = BoxDataset(priors=priors, size=size, survey_framework=survey_framework, model_framework=model_framework)
#dt_train.make_dataset()
#file_name = os.path.join(save_loc, f"trainset_3.pkl")
#with open(file_name, 'wb') as file:
#    pkl.dump(dt_train, file)
#print(f"Data set of size {size} made and saved as {file_name}.")
