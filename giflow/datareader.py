...
class DataReader():
    ...
    def split_filenames(self, chunk_size=None, randomise=False):
        """
        Function to slip the list of filenames into chunks. Useful when want to read data in batches, not all at once.
        Parameters:
        -----------
            chunk_size : int
                The size of each section of filenames.
                If not given, then read from the class. 
                If none, then the whole list of filenames is returned as one list. (Default: None)
            randomise: bool
                If true, the list of filenames is shuffled
        Output:
        -------
            filenames: list
                List of lists, with each sublist of length chunk_size. 
        """
    ...
    def read_files(self, noise_augment=False, noise_augment_factor=2, noise_seed=None, noise_distribution=None):
        ...
        ----------
            ...
            noise_seed: int
                If given, then the noise for each gravity survey within the dataset will be generated with that seed. (Default: None)
            noise_distribution: Prior
                If it is given, the noise scale for each gravity survey will be drawn from this distribution. Has to be a Prior object. (Default: None)
        Output
        ------
            train_data: list
