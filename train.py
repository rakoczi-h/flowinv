...
survey_info_to_include = ['survey_width_ratio', 'density', 'noise_scale']
noise_distribution = Prior(distributions={'noise_scale': ['LogUniform', 1e-5, 2.0]})


# Reading in files
trainsize = 50000
dr_train = DataReader(filenames=[f"trainset_{n}.pkl" for n in range(0,5)], data_location=data, model_info_to_include=model_info_to_include, survey_info_to_include=survey_info_to_include, datasize=trainsize)
train_data, train_conditional = dr_train.read_files(noise_distribution=noise_distribution)

saveloc = os.path.join(save, 'train_examples/')
if not os.path.exists(saveloc):
    os.mkdir(saveloc)


noise_samples = noise_distribution.sample(size=1000)
print(np.min(noise_samples))
print(np.max(noise_samples))
plt.hist(noise_samples)
plt.savefig(os.path.join(saveloc, 'noise_hist.png'))
plt.close()
