...
plt.hist(js_for_hist_shallow, density=False, bins=np.logspace(np.log10(0.0001), np.log10(0.6), 20), histtype='step', linewidth=1.0, range=(0.0, 0.6), color='k')
...
plt.hist(js_for_hist_deep, density=False, bins=np.logspace(np.log10(0.0001), np.log10(0.6), 20), histtype='step', linewidth=1.0, range=(0.0, 0.6), color='k')
plt.xscale('log')
plt.xticks(fontsize=15)
plt.yticks(fontsize=15)
plt.xlabel('JS Divegence', fontsize=20)
...
plt.figure(figsize=(10,10))
plt.vlines(cz_deep, ymin=np.min(np.c_[jdf, jdb], axis=1), ymax=np.max(np.c_[jdf, jdb], axis=1), color='black', linewidth=1.5, zorder=2)
plt.vlines(cz_shallow, ymin=np.min(np.c_[jsf, jsb], axis=1), ymax=np.max(np.c_[jsf, jsb], axis=1), color='black', linewidth=1.5, zorder=2)
plt.vlines(-0.375, ymin=-0.05, ymax=0.7, linestyle='--', linewidth=1.5, color='black')
plt.scatter(cz_deep, jdf, color='skyblue', marker='o', zorder=3, edgecolors='black', s=100, label='NF')
plt.scatter(cz_deep, jdb, color='mediumpurple', marker='o', zorder=3, edgecolors='black', s=100,  label='NS')
plt.scatter(cz_shallow, jsf, color='moccasin', marker='o', zorder=3, edgecolors='black', s=100)
plt.scatter(cz_shallow, jsb, color='indianred', marker='o', zorder=3, edgecolors='black', s=100)
plt.legend(loc='upper left', fontsize=15)
plt.xticks(fontsize=15)
plt.yticks(fontsize=15)
plt.ylabel('JS Divergence with Prior', fontsize=20)
plt.xlabel(r'$c_z$ [m]', fontsize=20)
plt.ylim(-0.05, 0.7)
plt.grid(zorder=-1.0, linestyle="--")
plt.savefig(os.path.join(flow_location, "js_cz_scatter_1.png"))
plt.close()
